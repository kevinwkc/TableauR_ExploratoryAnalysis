...
Dans chaque cas j'essaye de penser mon code R pour qu'il puisse s'adapter à un maximum de situations. Cependant, n'hésitez pas à [me contacter](mailto:skeith@actinvision.com) si vous avez des difficultés.  
<br>  
...
> __Note importante__ : après une analyse des performances dans les communications entre _Rserve_ et _Tableau_, je me suis rendu compte que cette approche n'est pas du tout optimale. En effet, lorsque je découpe ma chaine de caractères dans Tableau pour recomposer le résultat, Tableau va relancer le script R pour chaque élément (contrairement à ce que je pensais). Ainsi, le script va ici tourner au moins 4 fois, une fois pour chaque élément : la courbe de régression, les 2 limites de l'intervalle de confiance ainsi que le résumé du modèle. Créer 4 champs calculés contenant les scripts R dans Tableau reviendrait donc au même. En fait, ce serait même plus performant puisque j'alourdis le tout en faisant appel à la fonction `paste0` dans R et en parsant la chaine de caractère de nombreuses fois dans Tableau.  
> Lorsque j'aurai plus de temps, j'essaierai de trouver une meilleure solution. Ce problème se pose dans toutes mes visualisations.  
> Pour info, j'ai aussi posté la question dans la [communauté Tableau](https://community.tableau.com/message/480972).  
...
# return as a string that will be parsed in tableau: first the 5 boxplot stats 
# and then the confidence interval for the median
invisible(paste0(
