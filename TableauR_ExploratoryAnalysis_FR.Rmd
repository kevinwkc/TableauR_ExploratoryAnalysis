...
À ce stade le plus dur est fait. Amenez vos trois vues dans un tableau de bord, ajoutez filtres et paramètres, puis mettez en forme à votre convenance. Faites attention à la largeur / hauteur des axes de vos vues. Par exemple, la largeur de l'axe des ordonnées sur le nuage de points doit être la même que celle de l'axe de gauche sur la vue des densités marginales de __X__.  
...
# format p value
pvalue_str <- if(test$p.value <= 2.2e-16) {
      paste0("p-value: < ", formatC(2.2e-16, digits=1, format="e"))
} else if(test$p.value >= 0.001) {
      paste0("p-value: ", formatC(test$p.value, digits=4, format="f"))
} else {
      paste0("p-value: ", formatC(test$p.value, digits=1, format="e"))
}

# return data to Tableau
invisible(paste0(test$estimate, "|P|", test$p.value, "|S|", paste0(capture.output(test), collapse="\n")))
```
